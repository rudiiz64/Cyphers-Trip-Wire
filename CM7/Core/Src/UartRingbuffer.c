 ...
 */
void Uart_write(int c, UART_HandleTypeDef *huart)
{
	if (c>=0)
	{
		...
		__HAL_UART_ENABLE_IT(huart, UART_IT_TXE); // Enable UART transmission interrupt
	}
}
 ...
 */
void Uart_sendstring (const char *s, UART_HandleTypeDef *huart)
{
	while(*s) Uart_write(*s++, huart);
}
